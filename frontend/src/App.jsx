...
// ── Error serialiser — handles string, array (FastAPI 422), or object ─────────
function errMsg(e) {
  if (typeof e === 'string') return e
  if (Array.isArray(e)) return e.map(x => x?.msg || JSON.stringify(x)).join('; ')
    ...
    <div style={{ marginBottom: 2 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6,
        ...
        <span onClick={() => setOpen(o => !o)}
          style={{ color: '#4a7a9b', display: 'flex', alignItems: 'center', width: 14, cursor: 'pointer' }}>
      ...
      {(open || group === null) && screens.map(screen => (
        <div key={screen} onClick={() => onToggleScreen(screen)}
  ...
  return (
    <div style={{
      ...
      <div style={{ fontFamily: 'Arial Black, sans-serif', fontWeight: 900,
        fontSize: 22, color: '#0056a0', letterSpacing: '-1px', lineHeight: 1 }}>OVIVO</div>
      <div style={{ fontFamily: 'Arial, sans-serif', fontSize: 7.5,
        color: '#0056a0', textAlign: 'center', lineHeight: 1.2, marginTop: 2 }}>
        Ultra-Pure Water +<br /><span style={{ color: '#00a0c0' }}>by Ecolab</span>
      </div>
    </div>
  )
}
...
export default function App() {
  ...
  // Per-file upload statuses: null | 'uploading' | 'ok' | 'error'
  const [statusStructure, setStatusStructure] = useState(null)
  const [statusPageCfg,   setStatusPageCfg]   = useState(null)
  const [statusViews,     setStatusViews]      = useState(null)
  const [statusSVGs,      setStatusSVGs]       = useState(null)
  ...
  const uploadSingleRef = async (file, refType, setStatus) => {
    setStatus('uploading')
    try {
      ...
      const detected = res.files?.[0]?.detected_as
      if (detected === 'unknown') {
        addLog(`Warning: could not auto-detect type for ${file.name} — uploaded as ${refType}`, 'error')
      } else {
        addLog(`✓ ${file.name} uploaded as "${detected}"`, 'normal')
      }
      setStatus('ok')
    } catch (e) {
      addLog(`Error uploading ${file.name}: ${errMsg(e)}`, 'error')
      setStatus('error')
    }
  }
  ...
  const handleStructureSelect = async (files) => {
    ...
    try {
      ...
      addLog(`✓ Structure file: ${file.name}`, 'info')
      setStatusStructure('ok')
    } catch (e) {
      addLog(`Error uploading structure: ${errMsg(e)}`, 'error')
      setStatusStructure('error')
    } finally {
      setLoading(false)
    }
  }

  // ── Individual ref file handlers ────────────────────────────────────────────
  const handlePageCfgSelect = async (files) => {
    const file = files[0]; if (!file) return
    setLoading(true)
    await uploadSingleRef(file, 'page_config', setStatusPageCfg)
    setLoading(false)
  }

  const handleViewsSelect = async (files) => {
    const file = files[0]; if (!file) return
    setLoading(true)
    await uploadSingleRef(file, 'views', setStatusViews)
    setLoading(false)
  }

  const handleSVGsSelect = async (files) => {
    const file = files[0]; if (!file) return
    setLoading(true)
    await uploadSingleRef(file, 'svgs', setStatusSVGs)
    setLoading(false)
  }
  ...
  const handleLoad = async () => {
    ...
    try {
      ...
      addLog(`Load failed: ${errMsg(e)}`, 'error')
    } finally {
      setLoading(false)
    }
  }
  ...
  const toggleScreen = (screen) =>
    setSelected(prev => { const n = new Set(prev); n.has(screen) ? n.delete(screen) : n.add(screen); return n })

  const toggleGroup = (screens, allChecked) =>
    setSelected(prev => { const n = new Set(prev); screens.forEach(s => allChecked ? n.delete(s) : n.add(s)); return n })
  ...
  const toggleAll   = () => setSelected(allChecked ? new Set() : new Set(allScreens))

  // ── Generate ────────────────────────────────────────────────────────────────
  const handleGenerate = async () => {
    if (!sessionId)           { addLog('No session active.', 'error'); return }
    if (selected.size === 0)  { addLog('Select at least one screen.', 'error'); return }
    if (!genPageConfig && !genNavScreen) { addLog('Enable at least one output type.', 'error'); return }
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{ width: 320, flexShrink: 0, borderRight: '1px solid #0d2030',
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '8px 8px' }}>
            {tree.length === 0
              ...
              : tree.map((g, i) => (
                <TreeGroup key={i} group={g.group} screens={g.screens}
                  selected={selected} onToggleGroup={toggleGroup} onToggleScreen={toggleScreen} />
              ))
            }
          </div>
        </div>
        ...
        <div style={{ flex: 1, padding: '20px 24px', display: 'flex', gap: 24 }}>
          ...
          <div style={{ width: 240, flexShrink: 0 }}>
            ...
            <Divider label="INPUT FILES" />
            <FileButton label="Select Structure File" accept=".xlsx"
              onSelect={handleStructureSelect} status={statusStructure} />

            <Divider label="REFERENCE ZIPS" />
            <FileButton label="Page Config Ref (.zip)" accept=".zip"
              onSelect={handlePageCfgSelect} status={statusPageCfg} />
            <FileButton label="Views Ref (.zip)" accept=".zip"
              onSelect={handleViewsSelect} status={statusViews} />
            <FileButton label="SVGs Ref (.zip) — optional" accept=".zip"
            ...
            <ActionButton
              ...
              onClick={handleGenerate} variant="success"
